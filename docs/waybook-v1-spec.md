...
| Fonts | **System default only — never bundle a paid font.** Hammerhead's data-field system uses Relative (Colophon) + Ping (both commercial — cannot ship in a free public APK) and **Inter** (free, OFL) for labels. We use the device's system font (inherits the native look for free, no license). If deterministic typography is ever wanted, the *only* legal bundle is **Inter** (matches Hammerhead's label face). Relative/Ping are off-limits. |
