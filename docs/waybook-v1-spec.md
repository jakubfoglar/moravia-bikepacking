...
| Fonts | The shipped Karoo font is very likely **IBM Plex** (free — OFL/Apache), per on-device inspection. (The Hammerhead *design* file uses Relative + Ping, both **commercial** — never bundle those.) Options, both legal: (a) **system default** — inherits the native font for free, no bundle (current approach, honours the earlier "no custom fonts" call); or (b) **bundle IBM Plex** (Sans for text, Mono for numeric values) for an exact, deterministic native match on every device. Decide in the P5/P6 typography pass. Inter is an acceptable free stand-in if IBM Plex proves wrong. |
...
2. **Resolve** the candidate to a real entity (research §Matching): search radius scaled by type, score on name + distance + type-consistency → confidence tier A–D. Tiers decide render: A/B = photo + paragraph; C = name + category + facts; D = drop or list-only. **Never fabricate.** Resolver candidates to evaluate in P3: Overpass point-query, Nominatim, and **Photon (Komoot's OSM geocoder — photon.komoot.io, free/no-key, self-hostable)** — Photon is strong at *name-near-coordinate* search, so it's the natural fit for `Na trase` waypoints (name known, no `wikidata`) and for snapping a slightly-off pin to the real OSM entity. Benchmark it against Nominatim on hit-rate + rate limits during P3.
